...
#### Scenario: 探測模態匯出對話框
- **GIVEN** 使用者已手動開啟任一匯出對話框，該對話框為前景視窗
- **WHEN** 使用者雙擊 `2a`／`2b`／`2c` 其中一支
- **THEN** 報告檔含該對話框的控制項節點，包含輸出路徑輸入框與確認按鈕

#### Scenario: 三種格式的報告互不覆蓋
- **GIVEN** 使用者依序探測了三種匯出格式的對話框
- **WHEN** 檢視 `probe-output/`
- **THEN** 三份報告的檔名各自帶有格式標籤（zip／aama／astm），可據以辨識來源格式

> **背景**：三種格式是三個不同的對話框，期二要各自操作。報告若無法辨識來源，
> 等於沒有探測——而檔名只差時間戳的話，靠執行順序推斷並不可靠（某一種很可能重跑過）。
